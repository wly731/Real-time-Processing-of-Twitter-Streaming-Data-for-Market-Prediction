...
class ANFIS:
    ...
    def LSE(self, A, B, initialGamma = 1000.):
        ...
        for i in range(len(coeffMat[:,0])): # iterate over all examples
            a = coeffMat[i,:]
            b = np.array(rhsMat[i])
    ...
    def trainHybridJangOffLine(self, epochs=5, tolerance=1e-5, initialGamma=1000, k=0.01):
        ...
        while (epoch < epochs) and (convergence is not True):
            ...
            print(f"epoch: {epoch}")
            print(f"Converged yet?: {convergence}")
...
def forwardHalfPass(ANFISObj, Xs):
    ...
    for pattern in range(len(Xs[:,0])): #iterate over all examples
        #layer one
        layerOne = ANFISObj.memClass.evaluateMF(Xs[pattern,:])

        #layer two
        # miAlloc: all permutations of different variable's MFs value
        # AND operation:  multiply the two values of 2 different variables' MFs
        ...
        if pattern == 0:
            w = layerTwo
        else: #stack new result from this example to the existing AND layer
    ...
    # final processing, split into array of arrays
    # each array correspond to 1 example
    # each example has w1*x1, w1*x2, w1*1
    #                  w2*x1, w2*x2, w2*1 ...
